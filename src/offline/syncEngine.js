...
async function applyQueueItem(item) {
  ...
  if (item.action === "update") {
    ...
    // می‌دهد (نه خطا) — این می‌تواند به این معنا باشد که رکورد سمتِ سرور در
    // این فاصله حذف شده، یا اینکه RLS دیگر آن را برایِ این کاربر قابل‌مطابقت
    // نمی‌داند (مثلاً تغییرِ دسترسی/شرکت وسطِ کار) — از رویِ همین پاسخ قابلِ
    // تفکیک نیست، پس پیام عمداً هر دو حالت را پوشش می‌دهد، نه فقط «حذف شده».
    // بدونِ این چک، این حالت به‌اشتباه «موفق» تلقی می‌شد: آیتم از صف حذف
    // می‌شد و ویرایشِ کاربر با وضعیتِ «همگام‌شده» کش می‌شد، درحالی‌که هیچ‌جا
    // واقعاً ذخیره نشده بود.
