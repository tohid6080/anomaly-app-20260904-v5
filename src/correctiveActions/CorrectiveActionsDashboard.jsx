...
export default function CorrectiveActionsDashboard({ onBack, currentUser, wide }) {
  ...
  // طبقِ ممیزیِ QA: اقداماتِ اصلاحی همین جلسه به Offline Queue وصل شدند
  // ولی هیچ نشانگرِ Sync ای در لیست نبود — کاربر نمی‌توانست بفهمد رکورد
  // هنوز در انتظارِ همگام‌سازی است یا شکست خورده.
  const retrySync = async (recordId) => {
    const queue = await getQueue();
    const item = queue.find((q) => q.module === "correctiveActions" && q.recordId === recordId);
  ...
  // طبقِ ممیزیِ QA: فایل همان لحظه‌ی انتخاب به Storage آپلود می‌شود (نه در
  // Commitِ نهایی) — اگر کاربر قبل از Save حذفش کند، بدونِ این فراخوانی
  // فایل برایِ همیشه یتیم در Storage می‌ماند.
  const removeAttachment = (idx) => {
    const att = form.attachments[idx];
    setForm((prev) => ({ ...prev, attachments: prev.attachments.filter((_, i) => i !== idx) }));
    const parsed = att?.url ? parseStorageUrl(att.url) : null;
            ...
            <div style={{ flex: 1, minWidth: 200 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                ...
                {a.syncStatus && a.syncStatus !== "synced" && (
                  <SyncStatusBadge status={a.syncStatus} onRetry={(e) => { e.stopPropagation(); retrySync(a.id); }} />
                )}
              </div>
