...
const HSE_MODULES = [
  ...
  {
    key: "contractorHseEvaluation",
    label: "ارزیابی عملکرد HSE پیمانکاران",
    labelKey: "moduleContractorHseEvaluation",
    icon: true,
  },
  {
...
function LoginScreen({ onLogin }) {
  ...
  const loginCardProps = {
    ...
    error, warning, loading, handleSubmit,
    bioAvailable, bioChecking, handleBiometricLogin,
    showTrialRequestBtn: landingButtons.loginTrialRequest,
    onTrialRequest: () => { setShowLogin(false); setShowTrialRequest(true); },
  };
      ...
      {loginOnly ? (
        ...
        <>
          ...
          {showLogin && (
            ...
            >
              <LoginFormCard {...loginCardProps} onClose={() => setShowLogin(false)} onClick={(e) => e.stopPropagation()} />
            </div>
          )}
        </>
      )}
...
// کارتِ فرمِ ورود — هم داخلِ Modal (حالتِ «نمایشِ کامل»، رویِ صفحه اصلی باز
// می‌شود، با دکمه‌ی X و onClick برایِ stopPropagation) هم به‌عنوانِ تکِ
// محتوایِ صفحه (حالتِ «فقط صفحه‌ی ورود») استفاده می‌شود، تا فیلدها/منطقِ
// ورود دوبار نوشته نشوند — نگاه کن به LoginScreen و «حالتِ نمایشِ صفحه
// اصلی» در SuperAdmin.
function LoginFormCard({
  ...
  error, warning, loading, handleSubmit,
  bioAvailable, bioChecking, handleBiometricLogin,
  showTrialRequestBtn, onTrialRequest, onClose, onClick,
}) {
  return (
    <div
      onClick={onClick}
    ...
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 2 }}>
        ...
        {onClose && (
          <button type="button" onClick={onClose} aria-label={t("commonCancel")} style={{ background: "none", border: "none", cursor: "pointer", color: THEME.text3, padding: 2 }}>
            <X size={18} />
          </button>
        )}
      </div>
...
function EmployerDashboard({ onLogout, currentUser }) {
  ...
  // صفحهٔ ترکیبیِ وب برای «مدیریت ماشین‌آلات»: Sub Header + فرمِ ثبت
  // (فیلدهای افقی) که بخشِ مدارک هم بلافاصله زیرِ همان فرم ظاهر می‌شود +
  // سپس لیستِ ماشین‌آلات. فرمِ ثبت برای کارفرما هم فعال است.
  ...
  const machineryWebCombined = (
    <div style={{ direction: dir }}>
      ...
      <MachineryDashboard
        key={`machinery-list-${webListRefresh}`}
        wide
        hideCreate
        currentUser={currentUser}
        role="EMPLOYER"
        readOnly={machineryReadOnly}
  ...
  // صفحهٔ ترکیبیِ وب برای «مدیریت ورود و تردد پرسنل»: Sub Header + فرمِ ثبتِ
  // پرسنل (فیلدهای افقی) + لیستِ پرسنل زیرِ آن.
  ...
  const personnelWebCombined = (
    <div style={{ direction: dir }}>
      ...
      <PersonnelDashboard
        key={`personnel-list-${webListRefresh}`}
        wide
        hideCreate
        currentUser={currentUser}
        role="EMPLOYER"
    ...
    >
      ...
      {view === "contractorHseEvaluation" && (
        <ContractorEvalDashboard
          ...
          currentUser={currentUser}
          readOnly={getAccessLevel(permMap, "contractorHseEvaluation") === "view"}
          onBack={() => setView("menu")}
        />
      )}
      {view === "hseSurvey" && (
...
function ContractorDashboard({ onLogout, currentUser }) {
  ...
  // ---- صفحاتِ ترکیبیِ وب برای پنلِ پیمانکار (مثلِ پنلِ کارفرما) ----
  const [webListRefresh, setWebListRefresh] = useState(0);
  const anomalyReadOnly = getAccessLevel(permMap, "anomalyReport") === "view";
  const personnelReadOnly = getAccessLevel(permMap, "personnelAccess") === "view";
  const machineryReadOnly = getAccessLevel(permMap, "machineryManagement") === "view";
