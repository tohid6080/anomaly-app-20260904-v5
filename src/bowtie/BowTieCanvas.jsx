...
// نیم‌عرض/نیم‌ارتفاع هر نوع گره — برای تشخیص هم‌پوشانی، باید با اندازه‌ی
// واقعی هرکدوم توی کامپوننت رندرشون (RectNode/BarrierNode/...) یکی باشه
const NODE_HALF = {
  threat: { w: NODE_W / 2, h: NODE_H / 2 },
  consequence: { w: NODE_W / 2, h: NODE_H / 2 },
  barrier: { w: 46, h: 30 },
  escalationFactor: { w: 34, h: 20 },
  escalationControl: { w: 34, h: 18 },
  topEvent: { w: 90, h: 40 },
};

// بعد از رها کردن یک گره، اگه با گره‌ی دیگه‌ای هم‌پوشانی داشت (به‌جای اینکه
// اجازه بدیم دقیقاً روی هم یا داخل هم بیفتن)، با کمترین جابه‌جایی ممکن در
// جهت محور با کمترین هم‌پوشانی، از هم جدا می‌شن — چند بار تکرار می‌شه تا
// هم‌پوشانی‌های زنجیره‌ای هم حل بشن
function resolveOverlap(draggedId, x, y, half, boundsMap) {
  let cx = x, cy = y;
  const PADDING = 8;
  for (let iter = 0; iter < 8; iter++) {
    let adjusted = false;
    for (const [otherId, b] of Object.entries(boundsMap)) {
      if (otherId === draggedId) continue;
      const dx = cx - b.x;
      const dy = cy - b.y;
      const overlapX = half.w + b.half.w + PADDING - Math.abs(dx);
      const overlapY = half.h + b.half.h + PADDING - Math.abs(dy);
      if (overlapX > 0 && overlapY > 0) {
        if (overlapX < overlapY) cx += dx >= 0 ? overlapX : -overlapX;
        else cy += dy >= 0 ? overlapY : -overlapY;
        adjusted = true;
      }
    }
    if (!adjusted) break;
...
export default function BowTieCanvas({ bowtie, threats, consequences, barriers, escalationFactors, escalationControls, onDataChange, readOnly }) {
  ...
  const registerBounds = (id, kind, x, y) => {
    nodeBoundsRef.current[id] = { x, y, half: NODE_HALF[kind] || { w: 40, h: 24 } };
  };
  ...
  const nodePos = (node, defaultX, defaultY) => ({
    x: node.posX != null ? node.posX : defaultX,
    y: node.posY != null ? node.posY : defaultY,
  });
  ...
  // شروع هر رندر، فهرست bounds را از صفر می‌سازیم (به‌جز Top Event که ثابت
  // است و همیشه به‌عنوان مانع باقی می‌ماند)
  nodeBoundsRef.current = { __topEvent__: { x: TOP_EVENT.x, y: TOP_EVENT.y, half: NODE_HALF.topEvent } };

  const resolvedBarrierPos = {};
  threatsForBowtie.forEach((t, i) => {
    const tp = displayPos(t.id, ...Object.values(threatPositions[i]));
    registerBounds(t.id, "threat", tp.x, tp.y);
    const chain = barriersFor("preventive", t.id);
    const xs = barrierPositions(tp.x + NODE_W / 2, TOP_EVENT.x - 80, chain.length);
    chain.forEach((b, j) => { const p = displayPos(b.id, xs[j], tp.y); resolvedBarrierPos[b.id] = p; registerBounds(b.id, "barrier", p.x, p.y); });
  });
  consForBowtie.forEach((c, i) => {
    const cp = displayPos(c.id, ...Object.values(consPositions[i]));
    registerBounds(c.id, "consequence", cp.x, cp.y);
    const chain = barriersFor("recovery", c.id);
    const xs = barrierPositions(TOP_EVENT.x + 80, cp.x - NODE_W / 2, chain.length);
    chain.forEach((b, j) => { const p = displayPos(b.id, xs[j], cp.y); resolvedBarrierPos[b.id] = p; registerBounds(b.id, "barrier", p.x, p.y); });
  });
  ...
  barriers.forEach((b) => {
    ...
    chain.forEach((f, i) => { const p = displayPos(f.id, xs[i], bp.y + ESC_DROP); resolvedFactorPos[f.id] = p; registerBounds(f.id, "escalationFactor", p.x, p.y); });
  });
  // موقعیت کنترل‌های تشدید داخل BarrierWithEscalation محاسبه می‌شود؛ همان‌جا
      ...
      <div style={{ position: "absolute", top: 10, insetInlineStart: 10, zIndex: 5, display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <button
          ...
          onClick={async () => {
            ...
            await recalculateAllBarriersForBowtie(bowtie.id);
              ...
              <React.Fragment key={t.id}>
                ...
                {chain.map((b) => (
                  <BarrierWithEscalation
                    ...
                    beginNodeDrag={beginNodeDrag} readOnly={readOnly} registerBounds={registerBounds}
...
function BarrierWithEscalation({ barrier, pos, factors, resolvedFactorPos, controlsFor, displayPos, selected, setSelected, beginNodeDrag, readOnly, onAddFactor, onAddControl, registerBounds }) {
  if (!pos) return null;
  return (
    <>
      <BarrierNode
      ...
      {factors.map((f) => {
        ...
        return (
          <React.Fragment key={f.id}>
            ...
            {controls.map((ctl, k) => {
              const ccp = displayPos(ctl.id, cxs[k], fp.y + ESC_DROP);
              registerBounds(ctl.id, "escalationControl", ccp.x, ccp.y);
              return (
