...
// اعتبارسنجیِ شماره‌موبایلِ ایران — الگویِ یکتا برایِ همه‌ی فرم‌ها
// (پیش از این در ۶ جا با ۳ نامِ متفاوت تکرار شده بود)
export function isValidMobile(phone) {
  return /^09\d{9}$/.test((phone || "").trim());
}

// ---------- توکن‌های طراحی (پالت و تایپوگرافی سازمانی) ----------
// پالت پایه — تمِ تیرهٔ نئونیِ نمونهٔ طراحی (زمینه #0B1A24، کارت #0F2A3A،
// تأکید #14B8A6). هر رنگ var(--ihms-*) است؛ مقادیرِ پیش‌فرض در main.jsx
// روی :root ست می‌شوند و اگر شرکت در «ظاهر سامانه» چیز دیگری تنظیم کرده
// باشد، applyAppearanceToDom همان را جایگزین می‌کند.
export const THEME = {
  navy: "var(--ihms-navy, #0a1620)",
  navyDeep: "var(--ihms-navy-deep, #07121a)",
  navyMid: "var(--ihms-navy-mid, #123a49)",
  teal: "var(--ihms-teal, #14b8a6)",
  tealDeep: "var(--ihms-teal-deep, #0f9488)",
  tealSoft: "var(--ihms-teal-soft, #0f2e2b)",
  bg: "var(--ihms-bg, #0b1a24)",
  surface: "var(--ihms-surface, #0f2a3a)",
  surface2: "var(--ihms-surface-2, #123240)",
  border: "var(--ihms-border, #1e3d4d)",
  borderSoft: "var(--ihms-border-soft, #17303c)",
  borderStrong: "var(--ihms-border-strong, #274a5c)",
  ...
  warnBg: "var(--ihms-warn-bg, #3a2c14)",
  ok: "var(--ihms-ok, #22c55e)",
  okBg: "var(--ihms-ok-bg, #173021)",
  accentSoft: "var(--ihms-teal-soft, #0f2e2b)",
  font: "var(--ihms-font)",

  // --- سیستمِ توکنِ حرفه‌ای (فاز ۲) ---
  // هر توکن var(--ihms-*, <پیش‌فرضِ برابرِ همان عددی که امروز در کد است>)
  // است؛ تا وقتی سوپرادمین چیزی تنظیم نکند، ظاهر ذره‌ای تغییر نمی‌کند.
  // ناحیه‌ها
  headerBg: "var(--ihms-header-bg, #0a1620)",
  headerBorder: "var(--ihms-header-border, #07121a)",
  sidebarBg: "var(--ihms-sidebar-bg, #0a1620)",
  sidebarBorder: "var(--ihms-sidebar-border, #1e3d4d)",
  cardBg: "var(--ihms-card-bg, #0f2a3a)",
  cardBorder: "var(--ihms-card-border, #1e3d4d)",
  widgetBg: "var(--ihms-widget-bg, #0f2a3a)",
  widgetBorder: "var(--ihms-widget-border, #1e3d4d)",
  // تایپوگرافی به تفکیکِ نقش
  fw: "var(--ihms-fw, 400)",
  fwHeading: "var(--ihms-fw-heading, 700)",
  fsHeader: "var(--ihms-fs-header, 13px)", fwHeader: "var(--ihms-fw-header, 700)",
  fsMenu: "var(--ihms-fs-menu, 14.5px)", fwMenu: "var(--ihms-fw-menu, 600)",
  fsTitle: "var(--ihms-fs-title, 17px)", fwTitle: "var(--ihms-fw-title, 800)",
  fsBody: "var(--ihms-fs-body, 13px)", fwBody: "var(--ihms-fw-body, 400)",
  fsCard: "var(--ihms-fs-card, 12px)", fwCard: "var(--ihms-fw-card, 700)",
  fsKpi: "var(--ihms-fs-kpi, 26px)", fwKpi: "var(--ihms-fw-kpi, 800)",
  fsTable: "var(--ihms-fs-table, 12.5px)", fwTable: "var(--ihms-fw-table, 600)",
  // هندسه و فاصله
  radiusCard: "var(--ihms-radius-card, 12px)",
  radiusBtn: "var(--ihms-radius-btn, 9px)",
  pad: "var(--ihms-pad, 14px)",
  gap: "var(--ihms-gap, 10px)",
  iconSize: "var(--ihms-icon-size, 16px)",
  iconStroke: "var(--ihms-icon-stroke, 2)",
  // ارتفاع/سایه (حالتِ بصری Flat / 2.5D / 3D)
  elev1: "var(--ihms-elev-1, 0 1px 2px rgba(15,42,63,0.04), 0 4px 14px -8px rgba(15,42,63,0.12))",
  elev2: "var(--ihms-elev-2, 0 1px 2px rgba(15,42,63,0.04), 0 12px 32px -12px rgba(15,42,63,0.14))",
